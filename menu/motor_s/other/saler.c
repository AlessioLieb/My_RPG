...
{
    if (i == MONEY_S && red->py->player_key.key_a && red->py->invent.money >= 50) {
        place_item(red->rm);
        red->py->invent.money -= 50;
        red->py->player_key.key_a = false;
    }
    if (i == BOMBS_S && red->py->player_key.key_a && red->py->invent.bomb >= 10) {
    ...
    if (i == KEYS_S && red->py->player_key.key_e && red->py->invent.key >= 10) {
