...
static void mouv_all(adv_t *all, sfVector2f player_pos, int type, room *rm, player *py)
{
    for (int i = 0; i < 10; ++i)
...
void move_enemies(enemies_t *enem_t, player *py, room *rm)
{
    ...
    while (enem_t->move_ti.timer_total > 50000) {
        mouv_all(enem_t->flying_adv, p_pos, 0, rm, py);
        mouv_all(enem_t->big_adv, p_pos, 1, rm, py);
        mouv_all(enem_t->little_adv, p_pos, 2, rm, py);
        mouv_all(enem_t->no_mouving_adv, p_pos, 3, rm, py);
